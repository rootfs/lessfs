...
#if QLZ5_VERSION_MAJOR != 1 || QLZ5_VERSION_MINOR != 5 || QLZ5_VERSION_REVISION != 0
...
#if QLZ5_COMPRESSION_LEVEL == 1 && defined QLZ5_PTR_64 && QLZ5_STREAMING_BUFFER == 0
...
{
    compr *retdata;
    qlz5_state_decompress *state_decompress =
        (qlz5_state_decompress *) malloc(sizeof(qlz5_state_decompress));

    if ( buflen <= 0 ) return NULL;
    retdata = s_zmalloc(sizeof(compr));
    if (buf[0] == 'R') {
        retdata->size = qlz5_size_decompressed((char *) &buf[1]);
        retdata->data = s_zmalloc(retdata->size);
        retdata->size =
            qlz5_decompress((char *) &buf[1], retdata->data,
                            state_decompress);
    } else {
...
{
    compr *retdata;
    qlz5_state_compress *state_compress =
        (qlz5_state_compress *) malloc(sizeof(qlz5_state_compress));

//    pthread_mutex_lock(&qlz5_mutex);
    retdata = s_zmalloc(sizeof(compr));

    // allocate "uncompressed size" + 400 for the destination buffer
    retdata->data = s_zmalloc(buflen + 400);
    retdata->size =
        qlz5_compress(buf, (char *) &retdata->data[1], buflen,
                      state_compress);
...
//    pthread_mutex_unlock(&qlz5_mutex);
    return retdata;
}

int qlz5_get_setting(int setting)
{
    switch (setting) {
    case 0:
        return QLZ5_COMPRESSION_LEVEL;
    case 1:
        return sizeof(qlz5_state_compress);
    case 2:
        return sizeof(qlz5_state_decompress);
    case 3:
        return QLZ5_STREAMING_BUFFER;
#ifdef QLZ5_MEMORY_SAFE
    case 6:
        return 1;
#else
    case 6:
...
#endif
    case 7:
        return QLZ5_VERSION_MAJOR;
    case 8:
        return QLZ5_VERSION_MINOR;
    case 9:
        return QLZ5_VERSION_REVISION;
    }
    return -1;
}

#if QLZ5_COMPRESSION_LEVEL == 1
...
static void reset_table_compress(qlz5_state_compress * state)
{
    int i;
    for (i = 0; i < QLZ5_HASH_VALUES; i++) {
#if QLZ5_COMPRESSION_LEVEL == 1
...
static void reset_table_decompress(qlz5_state_decompress * state)
{
    int i;
    (void) state;
    (void) i;
#if QLZ5_COMPRESSION_LEVEL == 2
    for (i = 0; i < QLZ5_HASH_VALUES; i++) {
        state->hash_counter[i] = 0;
    }
#endif
...
{
#if QLZ5_COMPRESSION_LEVEL == 2
    return ((i >> 9) ^ (i >> 13) ^ i) & (QLZ5_HASH_VALUES - 1);
#else
    return ((i >> 12) ^ i) & (QLZ5_HASH_VALUES - 1);
#endif
}
...
size_t qlz5_size_decompressed(const char *source)
{
    ui32 n, r;
    n = (((*source) & 2) == 2) ? 4 : 1;
...
size_t qlz5_size_compressed(const char *source)
{
    ui32 n, r;
    n = (((*source) & 2) == 2) ? 4 : 1;
...
size_t qlz5_size_header(const char *source)
{
...
static __inline void update_hash(qlz5_state_decompress * state,
...
#elif QLZ5_COMPRESSION_LEVEL == 2
    ui32 hash;
    unsigned char c;
    hash = hashat(s);
    c = state->hash_counter[hash];
    state->hash[hash].offset[c & (QLZ5_POINTERS - 1)] = s;
    c++;
...
#if QLZ5_COMPRESSION_LEVEL <= 2
static void update_hash_upto(qlz5_state_decompress * state,
...
static size_t qlz5_compress_core(const unsigned char *source,
                                 unsigned char *destination, size_t size,
                                 qlz5_state_compress * state)
{
    ...
    while (src <= last_matchstart) {
        if ((cword_val & 1) == 1) {
            ...
            fast_write((cword_val >> 1) | (1U << 31), cword_ptr,
                       CWORD_LEN);
            ...
            if (cached == 0 && o != OFFSET_BASE
                ...
                if (*(o + 3) != *(src + 3)) {
                    ...
                    if (*(o + (src - old_src)) == *src) {
                        ...
                        if (*(o + (src - old_src)) == *src) {
                            size_t q =
                                last_byte - UNCOMPRESSED_END - (src - 5) +
                                1;
...
#elif QLZ5_COMPRESSION_LEVEL >= 2
        {
            ...
            size_t remaining =
                ...
                255 ? 255 : (last_byte - UNCOMPRESSED_END - src + 1);
            (void) best_k;
            ...
            if (offset2 < src - MINOFFSET && c > 0
                ...
                matchlen = 0;
            for (k = 1; k < QLZ5_POINTERS && c > k; k++) {
                o = state->hash[hash].offset[k];
#if QLZ5_COMPRESSION_LEVEL == 3
            ...
            o = offset2;
            state->hash[hash].offset[c & (QLZ5_POINTERS - 1)] = src;
            c++;
            state->hash_counter[hash] = c;
            ...
            if (matchlen > 2 && src - o < 131071) {
                ...
                for (u = 1; u < matchlen; u++) {
                    hash = hashat(src + u);
                    c = state->hash_counter[hash]++;
                    state->hash[hash].offset[c & (QLZ5_POINTERS - 1)] =
                        src + u;
                }
                ...
                if (matchlen == 3 && offset <= 63) {
                    *dst = (unsigned char) (offset << 2);
                    ...
                    ui32 f =
                        ((matchlen - 2) << 2) | ((ui32) offset << 7) | 3;
                    fast_write(f, dst, 3);
                    dst += 3;
                } else {
                    ui32 f =
                        ((matchlen - 3) << 7) | ((ui32) offset << 15) | 3;
                    fast_write(f, dst, 4);
                    dst += 4;
                }
            } else {
    ...
    // min. size must be 9 bytes so that the qlz5_size functions can take 9 bytes as argument
    return dst - destination < 9 ? 9 : dst - destination;
}

static size_t qlz5_decompress_core(const unsigned char *source,
                                   unsigned char *destination, size_t size,
                                   qlz5_state_decompress * state,
                                   const unsigned char *history)
{
    const unsigned char *src =
        source + qlz5_size_header((const char *) source);
    ...
    const unsigned char *last_source_byte =
        source + qlz5_size_compressed((const char *) source) - 1;
    ...
    for (;;) {
        ...
        if ((cword_val & 1) == 1) {
            ...
            hash = (fetch >> 4) & 0xfff;
            offset2 =
                (const unsigned char *) (size_t) state->hash[hash].offset;
            ...
            if (matchlen >
                (ui32) (last_destination_byte - dst - UNCOMPRESSED_END +
                        1))
                return 0;
#endif
...
size_t qlz5_compress(const void *source, char *destination, size_t size,
                     qlz5_state_compress * state)
{
    size_t r;
    ui32 compressed;
...
#if QLZ5_STREAMING_BUFFER > 0
    if (state->stream_counter + size - 1 >= QLZ5_STREAMING_BUFFER)
#endif
    {
        reset_table_compress(state);
        r = base + qlz5_compress_core((const unsigned char *) source,
                                      (unsigned char *) destination + base,
                                      size, state);
#if QLZ5_STREAMING_BUFFER > 0
        reset_table_compress(state);
#endif
    ...
    *destination |= (QLZ5_COMPRESSION_LEVEL << 2);
    *destination |= (1 << 6);
    *destination |=
        ((QLZ5_STREAMING_BUFFER ==
          0 ? 0 : (QLZ5_STREAMING_BUFFER ==
                   100000 ? 1 : (QLZ5_STREAMING_BUFFER ==
                                 1000000 ? 2 : 3))) << 4);
...
size_t qlz5_decompress(const char *source, void *destination,
                       qlz5_state_decompress * state)
{
    size_t dsiz = qlz5_size_decompressed(source);

#if QLZ5_STREAMING_BUFFER > 0
    if (state->stream_counter + qlz5_size_decompressed(source) - 1 >=
        QLZ5_STREAMING_BUFFER)
#endif
    {
        if ((*source & 1) == 1) {
            ...
            dsiz =
                qlz5_decompress_core((const unsigned char *) source,
                                     (unsigned char *) destination, dsiz,
                                     state,
                                     (const unsigned char *) destination);
        } else {
            memcpy(destination, source + qlz5_size_header(source), dsiz);
        }
        state->stream_counter = 0;
        reset_table_decompress(state);
    }
        ...
        if ((*source & 1) == 1) {
            dsiz =
                qlz5_decompress_core((const unsigned char *) source, dst,
                                     dsiz, state, (const unsigned char *)
                                     state->stream_buffer);
        } else {
            memcpy(dst, source + qlz5_size_header(source), dsiz);
            reset_table_decompress(state);
        }
